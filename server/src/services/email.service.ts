...
export const sendIDPasswordEmail = async (to: string, adminPassword: string) => {
    const mailOptions = {
        from: email,
        to,
        subject: "Generated Password for user on AasPass",
        text: `Your Password for your registered user on AasPass's : Email - ${to}, Password - ${adminPassword}`,
        html: `<p>Your Password for your registered user on AasPass's : Email - <strong>${to}</strong>, Password - <strong>${adminPassword}</strong></p>
