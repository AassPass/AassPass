...
const AdList = ({ ads, setAds, editingAd, setIsAdEditing, setEditingAd }) => {
  ...
  async function handleResetAd(ad) {
    try {
      const response = await fetch(`/api/ads/${ad.adCode}/reset`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reset: true }), // Backend should handle what reset means
      });
  ...
  return (
    <div className="flex-1 overflow-auto rounded border border-blue-400 bg-white">
  <div className="max-w-[1200px]"> {/* Increased max width */}
    <table className="w-full text-left text-black border-collapse border border-blue-400"> {/* Blue border added */}
      <thead>
        ...
        {ads.length === 0 ? (
          ...
          ads.map(ad => (
            <tr key={ad.adCode} className="text-black  text-sm border-t text-center">
              <td className="py-3 px-6 border border-blue-400 whitespace-nowrap">{ad.adCode}</td>
              <td className="py-3 px-6 border border-blue-400 whitespace-nowrap">{ad.title}</td>
              <td className="py-3 px-6 border border-blue-400 whitespace-nowrap">{ad.startDate}</td>
              <td className="py-3 px-6 border border-blue-400 whitespace-nowrap">{ad.endDate}</td>
